...
export const Login=async(req,res)=>{
    ...
    try {
        ...
        if(!ExistedUser) return res.status(401).json({message:"Invalid Credentials"})
        const decodedPassword = await bcrypt.compare(password, ExistedUser.password)
        if(!decodedPassword) return res.status(401).json({message:"Invalid Credentials"})
...
export const Register=async(req,res)=>{
    ...
    if(!fullName || !email || !password || !profilePic) return res.status(401).json({message:"All fields are required"})
    try {
        const ExistedUser = await User.findOne({email})
        if(ExistedUser) return res.status(401).json({message:"User with this account already exist"})
        const hashedPassword = await bcrypt.hash(password,10)
        const uploadres = await cloudinary.uploader.upload(profilePic,{folder:"user-profiles"})
...
export const CommunityUsers =async(req,res)=>{
    ...
    try {
        ...
        if(users.length == 0) return res.status(400).json({message:"No Communtity Users Found"})
